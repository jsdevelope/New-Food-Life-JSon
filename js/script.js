window.addEventListener('DOMContentLoaded', function () {
   ...
   /* new Menu(
    ...
    const message = {
        loading: 'img/slider/Spinner-2.gif',
        success: `Спасибо, мы перезвоним вам... ${emoji}`,
        failure: `Произошла ошибка, попробуйте позже. ${emoji2}`
    };
    ...
    function bindPostData(form) {
        form.addEventListener('submit', (e) => {
            ...
            postData('http://localhost:3000/requests', JSON.stringify(object))
                ...
                .then(data => {
                    console.log(data);
                    ShowTnhModal(message.success);
                    statusMessage.remove();
                }).catch(() => {
                    ShowTnhModal(message.failure);
                }).finally(() => {
                    form.reset();
                });
